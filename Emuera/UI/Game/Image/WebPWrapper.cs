	...
	private byte[] AdvancedEncode(Bitmap bmp, WebPConfig config, bool info)
	{
		...
		try
		{
			...
			if (info)
			{
				...
				RuntimeHost.ShowInfo("Dimension: " + wpic.width + " x " + wpic.height + " pixels\n" +
...
internal sealed partial class UnsafeNativeMethods
{
	...
	static UnsafeNativeMethods()
	{
		try
		{
			NativeLibrary.SetDllImportResolver(typeof(UnsafeNativeMethods).Assembly, ResolveLibrary);
		}
	...
	internal static unsafe void CopyMemory(nint dest, nint src, uint count)
	{
	...
	{
		if (libraryName != "libwebp.dll" && libraryName != "libwebp_x86.dll")
			return nint.Zero;
	...
	private static string[] GetLibraryCandidates()
	{
		...
		if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
			baseNames = nint.Size == 4 ? ["libwebp_x86.dll", "libwebp.dll"] : ["libwebp.dll", "libwebp_x86.dll"];
		else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
			baseNames = ["libwebp.so", "libwebp.so.7", "libwebp.so.6"];
		else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
			baseNames = ["libwebp.dylib"];
		else
			baseNames = ["libwebp"];

		// Probe host-local paths first so standalone/offline bundles can ship their own libwebp.
		...
		string envDir = Environment.GetEnvironmentVariable("EMUERA_LIBWEBP_DIR");
		if (!string.IsNullOrWhiteSpace(envDir))
			searchDirs.Add(envDir.Trim());
		...
		candidates.AddRange(baseNames);

		// Keep order, remove duplicates.
		var dedup = new List<string>(candidates.Count);
