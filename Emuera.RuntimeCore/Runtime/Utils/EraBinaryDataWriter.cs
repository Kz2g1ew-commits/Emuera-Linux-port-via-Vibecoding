...
{
	private readonly bool useCompressedHeader;

	public EraBinaryDataWriter(FileStream fs, bool systemSaveInBinary = false, bool zipSaveData = false)
	{
		useCompressedHeader = systemSaveInBinary && zipSaveData;
		#region EM_私家版_セーブ圧縮
		// writer = new BinaryWriter(fs, Encoding.Unicode);
		if (useCompressedHeader)
		{
			ms = new MemoryStream();
	...
	{
		#region EM_私家版_セーブ圧縮
		if (useCompressedHeader && writer != null)
		{
