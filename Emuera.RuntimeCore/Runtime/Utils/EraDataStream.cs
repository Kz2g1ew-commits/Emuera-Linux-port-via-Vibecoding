...
internal static class EraSaveMessages
{
	public static string InvalidData => RuntimeHost.ResolveMessage("save.invalid", "Invalid save data");
	public static string NotImplemented => RuntimeHost.ResolveMessage("save.not_impl", "Not implemented");

	public static string InvalidNumeric(string token)
	{
		var format = RuntimeHost.ResolveMessage("save.invalid_num", "Invalid numeric value: {0}");
...
internal sealed class EraDataReader : IDisposable
{
	...
	{
		if (reader == null)
			throw new FileEE(EraSaveMessages.InvalidData);
			...
			while (true)
			{
				...
				for (int x = 0; x < tokens.Length; x++)
					if (!long.TryParse(tokens[x], out intTokens[x]))
						throw new FileEE(EraSaveMessages.InvalidNumeric(tokens[x]));
...
internal sealed class EraDataWriter : IDisposable
{
	...
	//}
	public EraDataWriter(FileStream file, Encoding saveEncoding)
	{
		this.file = file;
		writer = new StreamWriter(file, saveEncoding ?? Encoding.UTF8);
	}
	...
	{
		throw new NotImplementedException(EraSaveMessages.NotImplemented);
	}
